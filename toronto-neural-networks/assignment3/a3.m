...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.hid_to_class = delta_3 * hid_output' ./ m + wd_coefficient * model.hid_to_class;
  
  delta_2 = model.hid_to_class' * delta_3 .* hid_output .* (1 - hid_output);
  ret.input_to_hid = delta_2 * data.inputs' ./ m + wd_coefficient * model.input_to_hid;
